...
public class MappingUtil {
	...
	// 수의사 추가하기
	public static Vet getVetFromRequest(HttpServletRequest req, int vetNo, int hospitalNo) {
		Vet v = new Vet();
		v.setVetNo(vetNo);
		v.setHospitalNo(hospitalNo);

		String path = req.getServletContext().getRealPath("hospitals/vetimg");
		DiskFileItemFactory f = new DiskFileItemFactory();
		ServletFileUpload uploader = new ServletFileUpload(f);
		uploader.setFileSizeMax(1024 * 1024 * 10);
		List<FileItem> list;
		...
		try {
			...
			for (FileItem item : list) {
				...
				if (item.isFormField()) {
					if (item.getFieldName().equals("vet_name")) {
						v.setVetName(item.getString("UTF-8"));
					} else if (item.getFieldName().equals("vet_introduce")) {
						v.setVetIntroduce(item.getString("UTF-8"));
					}
				} else {
					String fileName = item.getName();
					// System.out.println(item.getName());
