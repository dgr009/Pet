...
<%
	Beauty h = (Beauty) session.getAttribute("beauty");
%>
<html>
<head>
		...
		<div class="modal-body">
			<div class="form-group">
				...
				<div class="form-group">
					...
					<div id="imagePreview">
						<img src='/ppp/beauties/beautyimg/<%=h.getBeautyPhoto()%>'
							width='150px'>
					</div>
				</div>
...
<script>
	...
	var commit_orner = function() {
		...
		$.ajax({
			type:'POST',
			url:'/ppp/beauty/beautyupdate',
			data: {beauty_name:$("#business_name").val(),beauty_phone:$("#orner_phone").val(),beauty_mail:$("#orner_mail").val(),beauty_address:$("#orner_address").val(),beauty_pwd:$("#orner_pwd").val()},
	...
	var rollback_orner = function() {
		businessName.value = "<%=h.getBeautyName()%>";
		ornerPhone.value = "<%=h.getBeautyPhone()%>";
		ornerMail.value = "<%=h.getBeautyMail()%>";
		ornerAddress.value = "<%=h.getBeautyAddress()%>";
		ornerPwd.value = "<%=h.getBeautyPwd()%>";
