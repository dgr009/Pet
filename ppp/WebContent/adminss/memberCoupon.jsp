...
$(document).ready(function() {
	...
	$.each(inactivememberList, function(index, h) {
		...
		var coupon = $("<td></td>").html(h.memberCoupon);
		var tr = $("<tr width='90%'></tr>").append(no).append(name).append(id).append(ornerNo).append(ornerName).append(phone).append(mail).append(address).append(coupon);
...
<header> 
 <%@ include file='../adminss/header/YesHeader.jsp' %>  
	...
	<form id="inline_form" action="/ppp//admin/membercouponlist" method="get">
		<button>검색</button>
	</form>
	<hr />
